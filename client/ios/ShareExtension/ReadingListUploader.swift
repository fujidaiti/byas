...
final class ReadingListUploader: NSObject {
  ...
  func upload(url: String, title: String?, token: String) {
    do {
      ...
      // Background sessions created from an app extension need a unique identifier per
      // session, so two shares in quick succession cannot collide. Runner reconnects with
      // whatever identifier the system hands it, so it need not be predictable.
      let configuration = URLSessionConfiguration.background(
        withIdentifier: "\(Bundle.main.bundleIdentifier!).\(UUID().uuidString)")
  ...
  private func writeBody(named name: String, url: String, title: String?) throws -> URL {
